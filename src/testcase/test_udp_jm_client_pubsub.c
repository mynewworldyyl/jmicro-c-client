...
static uint8_t test_onPubsubItemType1Listener(jm_pubsub_item_t *it) {
	msg_extra_data_t* ehost = extra_strKeyGet(it->cxt, EXTRA_SKEY_UDP_HOST);
	msg_extra_data_t* pport = extra_strKeyGet(it->cxt, EXTRA_SKEY_UDP_PORT);
	msg_extra_data_t* isUdphost = extra_strKeyGet(it->cxt, EXTRA_SKEY_UDP_ACK);
	...
	extra_putInt(ex, EXTRA_KEY_UDP_PORT,port);
	extra_putByte(ex, EXTRA_KEY_UDP_ACK,1);
